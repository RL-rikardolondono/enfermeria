...
export async function adminRoutes(app: FastifyInstance) {
  ...
  app.get('/profesionales/verificacion', { preHandler }, async () => {
    return prisma.profesional.findMany({
      ...
      include: { usuario: { select: { nombreCompleto: true, email: true, telefono: true } }, documentos: true },
  ...
  app.post('/profesionales/:id/aprobar', { preHandler }, async (request, reply) => {
    ...
    const { aprobado, motivo } = z.object({ aprobado: z.boolean(), motivo: z.string().optional() }).parse(request.body)
    const profesional = await prisma.profesional.update({
      where: { id },
      data: { estadoVerificacion: aprobado ? 'aprobado' : 'rechazado', verificadoPor: request.usuario.id, verificadoEn: new Date() },
    ...
    await notificarUsuario(profesional.usuarioId, {
      ...
      cuerpo: aprobado ? 'Tu perfil ha sido verificado. Ya puedes recibir solicitudes.' : `Tu verificación fue rechazada. ${motivo ?? ''}`,
      datos: { motivo },
    })
    return profesional
  })
  ...
  app.get('/reportes', { preHandler }, async (request) => {
    const { desde, hasta } = z.object({
      desde: z.string().default(() => { const d = new Date(); d.setMonth(d.getMonth() - 1); return d.toISOString() }),
    ...
    const [porEstado, porTipo, pagos, topProfesionales] = await prisma.$transaction([
      prisma.servicio.groupBy({ by: ['estado'], where: { createdAt: rango }, _count: true, orderBy: { _count: { estado: 'desc' } } }),
      prisma.servicio.groupBy({ by: ['tipo'], where: { createdAt: rango }, _count: true, orderBy: { _count: { tipo: 'desc' } } }),
      prisma.pago.aggregate({ where: { createdAt: rango, estado: 'aprobado' }, _sum: { monto: true }, _count: true, _avg: { monto: true } }),
      prisma.profesional.findMany({
        where: { estadoVerificacion: 'aprobado' },
        orderBy: { totalServicios: 'desc' },
        take: 10,
        select: { id: true, calificacionPromedio: true, totalServicios: true, usuario: { select: { nombreCompleto: true } } },
      }),
    ])
